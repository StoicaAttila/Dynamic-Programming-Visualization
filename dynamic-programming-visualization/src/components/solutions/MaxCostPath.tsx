...
async function findMaxCostPath(delay: number): Promise<number> {
    ...
    for (let i = 1; i < m; i++) {
        for (let j = 1; j < n; j++) {
            ...
            calculationBox.textContent = ` Max(${dp[i - 1][j].toString()}, ${dp[i][j - 1].toString()})`

            await new Promise((resolve) => setTimeout(resolve, delay));
            tableDP.rows[i - 1].cells[j].classList.remove("search");
            tableDP.rows[i].cells[j - 1].classList.remove("search");
            tableDP.rows[i].cells[j].textContent = Math.max(dp[i - 1][j], dp[i][j - 1]).toString();

            await new Promise((resolve) => setTimeout(resolve, delay));
            table.rows[i].cells[j].classList.add("search");

            calculationBox.textContent = ` Max(${dp[i - 1][j].toString()}, ${dp[i][j - 1].toString()}) + ${matrix[i][j].toString()}`
    ...
    // Return the maximum cost path
    calculationBox.textContent = ` The maximum cost path is: ${maxCost.toString()}`
    return maxCost;
}
